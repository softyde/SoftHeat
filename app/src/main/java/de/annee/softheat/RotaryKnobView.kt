...
class RotaryKnobView @JvmOverloads constructor(
    ...
    fun applyValue(value: Int, redrawKnob: Boolean = false) {

        val x: Float = -150.0f + 300.0f * ((value - minValue).toFloat() / (maxValue - minValue).toFloat())
        Log.w("debug", "$x");
        if(redrawKnob) {
            setKnobPosition(x)
        }
     ...
     * The 0,0 coo(rdinates in android are the top left corner of the view.
